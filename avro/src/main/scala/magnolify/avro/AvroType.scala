...
sealed trait AvroField[T] extends Serializable { self =>
  ...
  private[avro] def buildSchema(cm: CaseMapper): Schema
...
object AvroField extends AvroFieldInstance0 {
  ...
  class FromWord[T] {
    def apply[U](f: T => U)(g: U => T)(implicit af: AvroField[T]): AvroField[U] =
      new Aux[U, af.FromT, af.ToT] {
        override def buildSchema(cm: CaseMapper): Schema = af.schema(cm)
  ...
  def bigDecimal(precision: Int, scale: Int = 0): AvroField[BigDecimal] =
    AvroField.logicalType[Array[Byte]](LogicalTypes.decimal(precision, scale))(
      Decimal.fromBytes(_, precision, scale)
    )(Decimal.toBytes(_, precision, scale))

  def fixed[T: ClassTag](size: Int)(
    f: Array[Byte] => T
  )(
    g: T => Array[Byte]
  )(implicit an: AnnotationType[T]): AvroField[T] =
...
trait AvroFieldInstance0 extends AvroFieldInstance1 {
  private def aux[T, From, To](tpe: Schema.Type)(f: From => T)(g: T => To): AvroField[T] =
    new AvroField.Aux[T, From, To] {
      override def buildSchema(cm: CaseMapper): Schema = Schema.create(tpe)
  ...
  private def id[T](tpe: Schema.Type): AvroField[T] =
    aux[T, T, T](tpe)(identity)(identity)
  ...
  ): AvroField[C[T]] = new AvroField.Aux[C[T], ju.List[f.FromT], GenericArray[f.ToT]] {
    override def buildSchema(cm: CaseMapper): Schema =
  ...
  implicit def afMap[T](implicit f: AvroField[T]): AvroField[Map[String, T]] =
    new AvroField.Aux[Map[String, T], ju.Map[CharSequence, f.FromT], ju.Map[String, f.ToT]] {
      override def buildSchema(cm: CaseMapper): Schema =
  ...
  implicit val afUuid: AvroField[ju.UUID] =
    AvroField.logicalType[String](LogicalTypes.uuid())(ju.UUID.fromString)(_.toString)
...
trait AvroFieldInstance1 extends AvroFieldInstance2 {
  ...
  def join[T](caseClass: CaseClass[Typeclass, T]): AvroField[T] = {
    if (caseClass.isValueClass) {
      ...
      new AvroField[T] {
        ...
        override def buildSchema(cm: CaseMapper): Schema = tc.buildSchema(cm)
      ...
      new AvroField.Record[T] {
        override def buildSchema(cm: CaseMapper): Schema = Schema
          .createRecord(
            caseClass.typeName.short,
            AvroField.getDoc(caseClass.annotations, caseClass.typeName.full),
            ...
            caseClass.parameters.map { p =>
              new Schema.Field(
                ...
                AvroField.getDoc(p.annotations, s"${caseClass.typeName.full}#${p.label}"),
                p.default
...
trait AvroFieldInstance2 {
  ...
  implicit def afEnum[T](implicit et: EnumType[T]): AvroField[T] = {
    ...
    new AvroField.Aux[T, AnyRef, EnumSymbol] {
      override def buildSchema(cm: CaseMapper): Schema = {
        val doc = AvroField.getDoc(et.annotations, s"Enum ${et.namespace}.${et.name}")
