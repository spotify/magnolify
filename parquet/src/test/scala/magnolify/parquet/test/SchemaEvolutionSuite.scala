...
import org.apache.avro.generic.{GenericData, GenericEnumSymbol, GenericRecord, GenericRecordBuilder}
import org.apache.avro.{JsonProperties, Schema}
import org.apache.hadoop.conf.Configuration
import org.apache.parquet.avro.{AvroParquetReader, AvroParquetWriter, AvroReadSupport, GenericDataSupplier}
...
object SchemaEvolutionSuite {
  ...
  private val types1 = List("Checking", "Saving")
  private val types2 = types1 :+ "OffShore"
  val accountTypeSchema1: Schema =
    Schema.createEnum("AccountType", "", namespace, types1.asJava)
  val accountTypeSchema2: Schema =
    Schema.createEnum("AccountType", "", namespace, types2.asJava)
  ...
  val (userSchema1, userSchema2): (Schema, Schema) = {
    ...
    val type1 = new Schema.Field("account_type", accountTypeSchema1, "", null)
    val location1 = new Schema.Field("location", locationSchema1, "", null)
    v1.setFields(List(id, name, type1, location1).asJava)

    val v2 = Schema.createRecord("UserV2", "", namespace, false)
    val type2 = new Schema.Field("account_type", accountTypeSchema2, "", null)
    ...
    v2.setFields(List(id, name, type2, location2, email, aliases).asJava)
  ...
  def avroUser1(id: Long, name: String, tpe: String, country: String, state: String): GenericRecord =
    new GenericRecordBuilder(userSchema1)
      .set("id", id)
      .set("name", name)
      .set("account_type", new GenericData.EnumSymbol(accountTypeSchema1, tpe))
  ...
  def avroUser2(
    ...
    new GenericRecordBuilder(userSchema2)
      .set("id", id)
      .set("name", name)
      .set("account_type", new GenericData.EnumSymbol(accountTypeSchema2, tpe))
  ...
  val avro2as1: Seq[GenericRecord] =
    avro1 :+ avroUser1(4, "Ed", "OffShore", "US", "CA")
  ...
  val avro2: Seq[GenericRecord] = Seq(
    ...
    avroUser2(4, "Ed", "OffShore", "US", "CA", "98765", "ed@aol.com", Seq("Edward"))
  )
...
class SchemaEvolutionSuite extends MagnolifySuite {
  ...
  println("=" * 100)
  readAvro(avroBytes2, userSchema1).foreach(println)
  println("=" * 100)
  avro2as1.foreach(println)

  test("Avro V2 => Avro V1") {
    val ed1 = readAvro(avroBytes2, userSchema1).last
    val ed2 = avro2as1.last
    println("ED1 " + ed1)
    println("ED2 " + ed2)

    val tpe1 = ed1.get("account_type")
    val tpe2 = ed2.get("account_type")
    println("ED1 tpe " + tpe1)
    println("ED2 tpe " + tpe2)

    val s1 = tpe1.asInstanceOf[GenericEnumSymbol[_]].getSchema
    val s2 = tpe2.asInstanceOf[GenericEnumSymbol[_]].getSchema
    println("ED1 schema " + s1)
    println("ED2 schema " + s2)

    /// NPEs in EnumSchema.getEnumOrdinal
    println("ED1 ord " + s1.getEnumOrdinal(tpe1.toString))
    println("ED2 ord " + s2.getEnumOrdinal(tpe2.toString))
