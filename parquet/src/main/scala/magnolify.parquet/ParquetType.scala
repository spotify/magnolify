...
import magnolia._
import magnolify.shared.{Decimal, EnumType}
...
object ParquetType {
  // FIXME: case mapper
  implicit def apply[T](implicit f: ParquetField.Record[T]): ParquetType[T] = new ParquetType[T] {
    override def schema: Type = f.schema
    override def write(c: RecordConsumer, v: T): Unit = f.write(c, v)
...
sealed trait ParquetField[T] extends Serializable {
  ...
  protected def writeGroup(c: RecordConsumer, v: T): Unit = {
    if (isGroup) {
      c.startGroup()
    }
    write(c, v)
...
object ParquetField {
  ...
  class FromWord[T] {
    def apply[U](f: T => U)(g: U => T)(implicit pf: Primitive[T]): Primitive[U] =
      new Primitive[U] {
        override def schema: Type = pf.schema
        override def write(c: RecordConsumer, v: U): Unit = pf.write(c, g(v))
  ...
  def primitive[T](
    ...
    new Primitive[T] {
      override def schema: Type = Schema.primitive(ptn, lta)
      override def write(c: RecordConsumer, v: T): Unit = f(c)(v)
      override def newConverter: TypeConverter[T] = g
    }

  implicit val ptBoolean =
  ...
  implicit def ptOption[T](implicit t: ParquetField[T]): ParquetField[Option[T]] =
    new ParquetField[Option[T]] {
      ...
      override def write(c: RecordConsumer, v: Option[T]): Unit =
  ...
  implicit def ptIterable[T, C[T]](implicit
    ...
    new ParquetField[C[T]] {
      ...
      override def write(c: RecordConsumer, v: C[T]): Unit =
        v.foreach(t.writeGroup(c, _))

      override def newConverter: TypeConverter[C[T]] = {
        val buffered = t.newConverter.asInstanceOf[TypeConverter.Buffered[T]]
        new TypeConverter.Delegate[T, C[T]](buffered) {
  ...
  class LogicalTypeWord[T](lta: => LogicalTypeAnnotation) extends Serializable {
    def apply[U](f: T => U)(g: U => T)(implicit pf: Primitive[T]): Primitive[U] = new Primitive[U] {
      override def schema: Type = Schema.setLogicalType(pf.schema, lta)
      override def write(c: RecordConsumer, v: U): Unit = pf.write(c, g(v))
      override def newConverter: TypeConverter[U] =
        pf.newConverter.asInstanceOf[TypeConverter.Primitive[T]].map(f)
    }
  ...
  def decimalFixed(length: Int, precision: Int, scale: Int = 0): Primitive[BigDecimal] = {
    ...
    new Primitive[BigDecimal] {
      ...
      override def write(c: RecordConsumer, v: BigDecimal): Unit =
  ...
  implicit val ptUuid: Primitive[UUID] = new Primitive[UUID] {
    ...
    override def write(c: RecordConsumer, v: UUID): Unit =
      c.addBinary(
