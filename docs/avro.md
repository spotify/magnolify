...
Achiving backward compatibility when adding new fields to the case class: new fields must be of type `Option[_]` and have a default parameter value `None` in order to generate backward compatible Avro schema `avroType.schema`.

```scala
case class Record(oldField: String, newField: Option[String] = None)
```
