...
package object shims {
  ...
  object FactoryCompat extends LowPriorityFactoryCompat1 {
    ...
    def apply[A, C](b: => mutable.Builder[A, C]): FC[A, C] =
      new FactoryCompat[A, C] {
        override def newBuilder: mutable.Builder[A, C] = b
      }
  ...
  object SerializableCanBuildFroms {
    private def cbf[A, C](b: => mutable.Builder[A, C]): CanBuildFrom[C, A, C] =
      new CanBuildFrom[C, A, C] with Serializable {
        override def apply(from: C): mutable.Builder[A, C] = b
