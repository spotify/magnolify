...
object SemigroupDerivation {
  ...
  def join[T](caseClass: CaseClass[Semigroup, T]): Semigroup[T] = {
  ...
  def split[T: Dispatchable](sealedTrait: SealedTrait[Semigroup, T]): Semigroup[T] = ???

  implicit def gen[T]: Semigroup[T] = macro Magnolia.gen[T]

  @deprecated("Use gen instead", "0.7.0")
  def apply[T]: Semigroup[T] = macro Magnolia.gen[T]
}
